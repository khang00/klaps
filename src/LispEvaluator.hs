...
import LispVal hiding (body, closure, params)
import System.IO
import LispParser (readExpr, readExprList)
...
eval env (List [Atom "if", predicate, consequence, alt]) = do
...
makeFunc :: Monad m => Maybe String -> Environment -> [LispVal] -> [LispVal] -> m LispVal
makeFunc varargs env params body = return $ Func (map showVal params) varargs body env

makeNormalFunc :: Environment -> [LispVal] -> [LispVal] -> ExceptT LispError IO LispVal
makeNormalFunc = makeFunc Nothing

makeVarArgs :: LispVal -> Environment -> [LispVal] -> [LispVal] -> ExceptT LispError IO LispVal
...
cons [x, DottedList xs ls] = return $ DottedList (x : xs) ls
...
primitiveBindings :: IO Environment
primitiveBindings =
  nullEnv
    >>= ( flip bindVars $
